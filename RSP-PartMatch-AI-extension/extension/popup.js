...
function renderResults(data) {
  const originalStock = data.original_stock_options?.[0];
  const substitute = data.substitute_options?.[0] || data.substitutes?.[0];
  const hasSubstitute = data.recommendation_status === "substitutes_found" && substitute;
  const hasOriginalStock = data.recommendation_status === "original_stock_found" && originalStock;
  const hasApprovedResult = hasSubstitute || hasOriginalStock;

  results.innerHTML = `
    <div class="badge ${hasApprovedResult ? "success" : "warning"}">
      ${resultBadgeText(hasSubstitute, hasOriginalStock)}
    </div>
    ${renderOriginalPart(data.original_part)}
    ${hasOriginalStock ? renderOriginalStock(originalStock) : ""}
    ${hasSubstitute ? renderSubstitute(substitute) : ""}
    ${hasSubstitute ? renderComparison(data.comparison_table || []) : ""}
...
function resultBadgeText(hasSubstitute, hasOriginalStock) {
  if (hasSubstitute) return "● Approved substitute found";
  if (hasOriginalStock) return "● Original part found from approved vendor";
  return "● No approved match yet";
}
...
function renderOriginalStock(stockOption) {
  const title = [stockOption.manufacturer, stockOption.part_number].filter(Boolean).join(" ");

  return `
    <section class="card">
      <p class="card-title">Original Part Stock</p>
      <p class="part-title">${escapeHtml(title)}</p>
      <p class="meta">Vendor: <strong>${escapeHtml(stockOption.vendor)}</strong> · ${escapeHtml(stockOption.condition)} · ${escapeHtml(stockOption.stock_status || "Stock unknown")}</p>
      <div class="match-row">
        <div class="pill">
          <strong>Type</strong>
          Exact part
      ...
      <a class="link-button" href="${stockOption.product_link}" target="_blank" rel="noreferrer">Open vendor link →</a>
    </section>
  `;
}
